...
theorem chunk008_checked :
    packedCheck (restrictedAutoOracle restrictedTerms restrictedLogFuel)
      (follow restrictedLowerRoot chunk008Path) chunk008Packed = true := by
  rfl'

noncomputable def chunk008 :
    CheckedChunk (restrictedAutoOracle restrictedTerms restrictedLogFuel)
      (follow restrictedLowerRoot chunk008Path) :=
...
theorem chunk009_checked :
    packedCheck (restrictedAutoOracle restrictedTerms restrictedLogFuel)
      (follow restrictedLowerRoot chunk009Path) chunk009Packed = true := by
  rfl'

noncomputable def chunk009 :
    CheckedChunk (restrictedAutoOracle restrictedTerms restrictedLogFuel)
      (follow restrictedLowerRoot chunk009Path) :=
  PackedCheckedChunk.toCheckedChunk
...
def chunk010Path : Path :=
  [{ axis := .chi, branch := .lower }, { axis := .chi, branch := .lower }, { axis := .chi, branch := .upper }, { axis := .k, branch := .upper }, { axis := .k, branch := .upper }]
...
theorem chunk010_checked :
    packedCheck (restrictedAutoOracle restrictedTerms restrictedLogFuel)
      (follow restrictedLowerRoot chunk010Path) chunk010Packed = true := by
  rfl'

noncomputable def chunk010 :
    CheckedChunk (restrictedAutoOracle restrictedTerms restrictedLogFuel)
      (follow restrictedLowerRoot chunk010Path) :=
  PackedCheckedChunk.toCheckedChunk
...
def chunk011Path : Path :=
  [{ axis := .chi, branch := .lower }, { axis := .chi, branch := .upper }, { axis := .k, branch := .lower }, { axis := .chi, branch := .lower }, { axis := .k, branch := .lower }]
...
theorem chunk011_checked :
    packedCheck (restrictedAutoOracle restrictedTerms restrictedLogFuel)
      (follow restrictedLowerRoot chunk011Path) chunk011Packed = true := by
  rfl'

noncomputable def chunk011 :
    CheckedChunk (restrictedAutoOracle restrictedTerms restrictedLogFuel)
      (follow restrictedLowerRoot chunk011Path) :=
  PackedCheckedChunk.toCheckedChunk
