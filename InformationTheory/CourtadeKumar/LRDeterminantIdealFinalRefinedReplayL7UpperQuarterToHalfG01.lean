...
namespace L7UpperQuarterToHalfG01
...
def chunk004Path : Path :=
  [{ axis := .chi, branch := .lower }, { axis := .chi, branch := .lower }, { axis := .chi, branch := .upper }, { axis := .k, branch := .lower }]
...
theorem chunk004_checked :
    packedCheck (upperAutoOracle lowTerms lowLogFuel)
      (follow lowKUpperQuarterToHalfRoot chunk004Path) chunk004Packed = true := by
  rfl'

noncomputable def chunk004 :
    CheckedChunk (upperAutoOracle lowTerms lowLogFuel)
      (follow lowKUpperQuarterToHalfRoot chunk004Path) :=
...
theorem chunk005_checked :
    packedCheck (upperAutoOracle lowTerms lowLogFuel)
      (follow lowKUpperQuarterToHalfRoot chunk005Path) chunk005Packed = true := by
  rfl'

noncomputable def chunk005 :
    CheckedChunk (upperAutoOracle lowTerms lowLogFuel)
      (follow lowKUpperQuarterToHalfRoot chunk005Path) :=
  PackedCheckedChunk.toCheckedChunk
...
theorem chunk006_checked :
    packedCheck (upperAutoOracle lowTerms lowLogFuel)
      (follow lowKUpperQuarterToHalfRoot chunk006Path) chunk006Packed = true := by
  rfl'

noncomputable def chunk006 :
    CheckedChunk (upperAutoOracle lowTerms lowLogFuel)
      (follow lowKUpperQuarterToHalfRoot chunk006Path) :=
  PackedCheckedChunk.toCheckedChunk
...
theorem chunk007_checked :
    packedCheck (upperAutoOracle lowTerms lowLogFuel)
      (follow lowKUpperQuarterToHalfRoot chunk007Path) chunk007Packed = true := by
  rfl'

noncomputable def chunk007 :
    CheckedChunk (upperAutoOracle lowTerms lowLogFuel)
      (follow lowKUpperQuarterToHalfRoot chunk007Path) :=
  PackedCheckedChunk.toCheckedChunk
