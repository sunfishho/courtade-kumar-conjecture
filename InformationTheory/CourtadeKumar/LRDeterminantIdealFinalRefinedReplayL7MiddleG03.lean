...
def chunk012Path : Path :=
  [{ axis := .chi, branch := .lower }, { axis := .chi, branch := .upper }, { axis := .k, branch := .lower }, { axis := .k, branch := .lower }, { axis := .k, branch := .lower }]
...
theorem chunk012_checked :
    packedCheck (middleAutoOracle lowTerms lowLogFuel)
      (follow middleRoot chunk012Path) chunk012Packed = true := by
  rfl'

noncomputable def chunk012 :
    CheckedChunk (middleAutoOracle lowTerms lowLogFuel)
      (follow middleRoot chunk012Path) :=
...
def chunk013Path : Path :=
  [{ axis := .chi, branch := .lower }, { axis := .chi, branch := .upper }, { axis := .k, branch := .lower }, { axis := .k, branch := .lower }, { axis := .k, branch := .upper }]
...
theorem chunk013_checked :
    packedCheck (middleAutoOracle lowTerms lowLogFuel)
      (follow middleRoot chunk013Path) chunk013Packed = true := by
  rfl'

noncomputable def chunk013 :
    CheckedChunk (middleAutoOracle lowTerms lowLogFuel)
      (follow middleRoot chunk013Path) :=
  PackedCheckedChunk.toCheckedChunk
...
theorem chunk014_checked :
    packedCheck (middleAutoOracle lowTerms lowLogFuel)
      (follow middleRoot chunk014Path) chunk014Packed = true := by
  rfl'

noncomputable def chunk014 :
    CheckedChunk (middleAutoOracle lowTerms lowLogFuel)
      (follow middleRoot chunk014Path) :=
  PackedCheckedChunk.toCheckedChunk
...
def chunk015Path : Path :=
  [{ axis := .chi, branch := .lower }, { axis := .chi, branch := .upper }, { axis := .k, branch := .upper }, { axis := .k, branch := .lower }]
...
theorem chunk015_checked :
    packedCheck (middleAutoOracle lowTerms lowLogFuel)
      (follow middleRoot chunk015Path) chunk015Packed = true := by
  rfl'

noncomputable def chunk015 :
    CheckedChunk (middleAutoOracle lowTerms lowLogFuel)
      (follow middleRoot chunk015Path) :=
  PackedCheckedChunk.toCheckedChunk
