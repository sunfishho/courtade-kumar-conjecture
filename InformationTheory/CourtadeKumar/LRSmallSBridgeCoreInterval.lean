...
/-!
# Executable interval expression for the frozen small-`s` core

This is the arithmetic evaluator needed for the compact replay (M30).  Its
payload consists only of rational square-root and logarithm proposals; the
Boolean checker validates each proposal.  Boxes touching `chi = 0` use the
direct endpoint enclosure proved in `LRSmallSBridgeChi`.
-/
