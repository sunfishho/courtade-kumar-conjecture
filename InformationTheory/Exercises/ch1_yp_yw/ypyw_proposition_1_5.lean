...
noncomputable def shannonEntropyLogBase2 {k : ℕ} (p : Fin k → ℝ) : ℝ :=
  ∑ i : Fin k, Real.negMulLog (p i) * (Real.logb 2 (Real.exp 1))

theorem method_of_types {k : ℕ} (n : Fin k → ℕ) :
    let N : ℕ := ∑ i : Fin k, n i
    let p : Fin k → ℝ := fun i => (n i : ℝ) / (N : ℝ)
    ((Real.rpow 2 (N * shannonEntropyLogBase2 p)) / ((1 + (N : ℝ))^(k-1)))
        ≤ (multinomial_coeff n : ℝ)
      ∧ (multinomial_coeff n : ℝ) ≤ Real.rpow 2 (N * shannonEntropyLogBase2 p) := by
  sorry
